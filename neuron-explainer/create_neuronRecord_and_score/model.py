...
class HookManager:
    ...
    def _get_module_by_name(self, module, access_string):
        ...
        :param module: 模型的顶层模块。
        :param access_string: 模块的访问字符串，如 'transformer.h.0.mlp.gelu'。
        :return: 对应的模块。
        """
...
class GPT(nn.Module):
    ...
    # TODO! 在将句子输入给模型时，理论上应该加上句子开始token，即prepend_bos为True，但由于寻找最大激活值时只是简单地使用enc，故这里一样
    def to_tokens(self, text, prepend_bos):
        ...
        return torch.tensor([enc.encode_ordinary(text)])

    # TODO! 这是根据hookedtranformer文档中代码进行的简化版本
    def to_str_tokens(self, text, prepend_bos):
        enc = tiktoken.get_encoding("gpt2")
        decode = lambda l: enc.decode(l)
        if isinstance(text, str):
            tokens = self.to_tokens(text, prepend_bos=True)[0]
            str_list = [decode([token.item()]) for token in tokens]
            print
            return str_list
        else:
            print("error in model.to_str_tokens(): input is not str")
    ...
    def forward(self, idx, targets=None):
        ...
        for layer_idx, block in enumerate(self.transformer.h, 1):
            x = block(x)
        ...
        if targets is not None:
            ...
            if self.config.non_neg_weight:
                logits = self.non_neg_lm_head(x)
            else:
                logits = self.lm_head(x) # (b,t,vacob_size)
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
        else:
            # inference-time mini-optimization: only forward the lm_head on the very last position
            if self.config.non_neg_weight:
                logits = self.non_neg_lm_head(x[:, [-1], :]) # note: using list [-1] to preserve the time dim
            else:
    ...
    def run_with_cache(self, input_ids, layers_to_hook=None, device=None):
        """
        运行模型并捕捉指定层的激活值。

        :param input_ids: 输入的 token IDs，形状为 (batch_size, sequence_length)。
        :param layers_to_hook: 要捕捉的层的名称列表。如果为 None，则捕捉所有注意力层和 MLP 层的激活值。
