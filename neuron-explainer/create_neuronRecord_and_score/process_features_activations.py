...
print("Now processing activations in features layer")
...
for s_i in range(shard_save_num):
    ...
    for i in range(768):
        sequence_tokenized = tokenized_shard_part[
            activations[f"feature_{i}"]["max_example_index"]]['ids'][:max_length]
        first_sample_sequence_tokenized = tokenized_shard_part[0]['ids'][:max_length]
        max_token = decode([activations[f"feature_{i}"]["max_token_idx"]])
        if i == 0:
            print(max_token)
        # if sequence_tokenized not in info_dict[f"feature_{i}"]["max_sequence_tokenized"]:
        info_dict[f"feature_{i}"]["max_activation_value"].append(
            activations[f"feature_{i}"]["max_activation_value"])
        info_dict[f"feature_{i}"]["mean_activation_value"].append(
            activations[f"feature_{i}"]["mean_activation"])
        info_dict[f"feature_{i}"]["max_token"].append(
            max_token)
        info_dict[f"feature_{i}"]["max_token_idx"].append(
            activations[f"feature_{i}"]["max_token_idx"])
        info_dict[f"feature_{i}"]["max_sequence_tokenized"].append(
            sequence_tokenized)
        # info_dict[f"feature_{i}"]["max_sequence"].append(
        #     dataset_sharded[f"train_shard_{s_i}"][activations[f"feature_{i}"]["max_example_index"]]['text']) # 注意这里不能加[:max_length]
        # 上述代码保存的序列太长，长于max_length
        info_dict[f"feature_{i}"]["max_sequence"].append(
            [decode([sequence_tokenized[k]]) for k in range(max_length)])

        info_dict[f"feature_{i}"]["max_token_index"].append(
            activations[f"feature_{i}"]["max_token_index"])
        info_dict[f"feature_{i}"]["activations"].append(
            activations[f"feature_{i}"]["activations"])
        
        info_dict[f"feature_{i}"]["activations_first_sample"].append(
            activations[f"feature_{i}"]["activations_first_sample"])
        info_dict[f"feature_{i}"]["first_sample_sequence"].append(
            [decode([first_sample_sequence_tokenized[k]]) for k in range(max_length)])
        info_dict[f"feature_{i}"]["first_sample_sequence_tokenized"].append(
            first_sample_sequence_tokenized)
        info_dict[f"feature_{i}"]["num_info"] += 1
